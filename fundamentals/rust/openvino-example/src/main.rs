...
fn main() {
    ...
    let tensor_data = image2tensor::convert_image_to_tensor_bytes(
        "images/banana.jpg",
        224,
        224,
    ...
    let results = unsafe { results.buffer_as_type::<f32>().unwrap() };
    //let results = infer_request.get_blob(output_name).unwrap();
    let sorted_results = sort_results(results);
    // loop through the top 5 results
    for i in 0..5 {
        //println!("Result {}: {:?}", i, sorted_results[i]);
        let class = IMAGENET_CLASSES[sorted_results[i].0];
        let confidence = sorted_results[i].1;
        println!("Class: {}, Confidence: {}", class, confidence);
    }
}
