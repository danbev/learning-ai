...
int main(int argc, char **argv) {
    ...
    struct ggml_tensor * vertical_result = ggml_reshape_2d(ctx, result, 1, 3);
    struct ggml_tensor * transposed = ggml_transpose(ctx, result);
    /*
    [2, 3]        [2, 1]        [2, 3]     [1, 2]   [1, 3]
    +---+---+    +---+---+T    +---+---+   +---+    +---+
    | 1 | 2 |    | 7 | 8 |  =  | 1 | 2 |   | 7 |    | 23|
    ...
    ggml_mul_mat(a, b) produces {a->ne[1], b->ne[1]}
    ...
    By having the result tensor in this shape enables the following operations
    to be performed without having to reshape the result tensor.
    */
