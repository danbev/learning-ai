...
print(f'We want the following targets/labels to have a high probability:\n {ys})')
...
print("The following are the probabilities that were generated by the neuron for the true target values")
print("which are the indexes of the characters in the string 'emma'")
...
loss.backward()
#print(W.grad)
print(f'W.grad[0][0] has {W.grad[0][0]} influence on the loss. Because this is a')
print(f'positive value it would increase the loss.')
print(probs[torch.tensor([0, 1, 2, 3, 4]), ys].data)

W.data += -0.1 * W.grad.data

# The following is an example of doing the forward pass, followed by the the
# backward pass, followed by the update of the gradient. This is done multiple
# times manually which is what a gradient descent algorithm does.
