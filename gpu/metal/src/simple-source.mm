...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        // Load Metal source code from file
        NSError *error = nil;
        NSString *sourcePath = @"src/kernel.metal";
        NSURL *sourceURL = [NSURL fileURLWithPath:sourcePath];
        NSString *src = [NSString stringWithContentsOfURL:sourceURL encoding:NSUTF8StringEncoding error:&error];
        
        if (!src) {
            NSLog(@"Failed to load the kernel source. Error: %@", error.localizedDescription);
            return -1;
        }
        
        // Compile the Metal source code
        MTLCompileOptions *options = [[MTLCompileOptions alloc] init];
        options.languageVersion = MTLLanguageVersion2_4; // Adjust version as needed
        
        id<MTLLibrary> defaultLibrary = [device newLibraryWithSource:src options:options error:&error];
        
        if (!defaultLibrary) {
            NSLog(@"Failed to compile the Metal library. Error: %@", error.localizedDescription);
            return -1;
        }
