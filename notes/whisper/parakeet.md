...
### Chunking
We have chunking working for a single large file which is handled by
parakeet_full. This works by splitting the file into a left|center|right so
that the encoder has some context arounds its borders, where the chunk in the
center is what we are actually then using in the joint network.

But in a usecase like ffmpeg were the audio samples already come in chunks we
still need to enable the process to have some context around the edges or it
will not be able to correctly transcribe these parts.
```console
input  [                                                                  ]
            ↓
chunks [l | c | r ][          ][        ][       ][        ][      ][     ]
        ...
        \     /
         ...
         joint
```
If we store the right context in the state, we can use that as our left context
for the next chunk.
```console
input  [                                                                  ]
                +----  ↓
