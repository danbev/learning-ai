...
for this. Instead this is slapped on top of an exising model. So the idea is to
take an existing model which is frozen and train small additional heads on top.

I came across this when reading about MTP and just wanted to understand what this
...
Notice that the heads are all basing their predictions on the same hidden state
