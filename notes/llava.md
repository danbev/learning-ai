...
14x14 patches), and add a trainable projection layer (W) on top of the image and
text features for the model to learn how to combine them.
...
```
   ...
   +-------------+      +----------------+
```
But these embeddings from different embedding spaces, so we need to project them
into the same space. This is done by a trainable projection layer (W) on top of
```
...
encoder so that it can convert/transform the image embeddings to the same space
as the text embeddings. And the type of model has to be GGUF so this will also
...
Then we need to convert the llama part of llava to GGUF format, which we removed
